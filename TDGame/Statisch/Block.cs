...
namespace TDGame.Statisch
{
    ...
    public abstract class Block
    {
        ...
        public virtual bool schadenBekommen(int schaden, Projektileffekt projektileffekt)
        {
            ...
            else
            {
                if (schild < schaden)
                {
                    if (hP > (schaden - schild) * ruestung)
                    {
                        hP -= (int)(schaden * ruestung);
                        schild = 0;
                        return false;
                    }
                    else
        ...
        /// <summary>
        /// Die Clientversion für das schaden verursachen, hier wird im gegensatz zur Serverversion kein Gebaude zerstört, um die Synchronität zu gerwährleisten.
        /// </summary>
        /// <param name="schaden">Der schaden der clientintern verursacht wird aber nicht töten kann</param>
        /// <param name="projektileffekt">Der projektileffekt der entscheidet was der Treffer bewirkt.</param>
        /// <returns>gibt immer false zurück da es beim client nicht töten kann</returns>
        public virtual bool schadenBekommenClient(int schaden, Projektileffekt projektileffekt)
        {
            ...
            if (projektileffekt != Projektileffekt.schaden)
            {
                switch (projektileffekt)
                {
                    case Projektileffekt.energie:
                        ...
                        break;
                    case Projektileffekt.schild:
                        if (schild + schaden <= maxSchild)
                        {
                            schild += schaden;
                        }
                        else schild = maxSchild;
                        break;
                    case Projektileffekt.angriffsgeschwindigkeit:
                        if (turmD != null)
                        {
            ...
            else
            {
                if (schild < schaden)
                {
                    ...
                    {
                        schild = 0;
                        hP = 0;//Turm wäre theoretisch kaput. Client zerstört jedoch keine türme
                        return false;
                    }
                }
                else
